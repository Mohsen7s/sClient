...
void CDebugger::make_crashreport(int sig, const char *pFilePath, char *pOutMsgBoxTitle, unsigned TitleBufferSize, char *pOutMsgBoxMsg, unsigned MsgBufferSize)
{
	...
	else
	{
		...
		str_timestamp_format(aBuf, sizeof(aBuf), "### io.github.Sash::ath.crashreport.autogenerated %d.%m.%Y %H:%M:%S");
		WRITE_LINE();
	...
	str_format(pOutMsgBoxTitle, TitleBufferSize, "Sash Crash  --  %s", paFunnyMessages[n]);
}
