...
void CClient::SendInput()
{
	...
	if(m_DummyConnected)
	{
		...
		//else if(g_Config.m_ClDummyHookFly && false)
		//{
		//	vec2 Main = ((CGameClient *)GameClient())->m_LocalCharacterPos;
		//	vec2 Dummy = ((CGameClient *)GameClient())->m_aClients[m_LocalIDs[!g_Config.m_ClDummy]].m_Predicted.m_Pos;
		//	vec2 Dir = Main - Dummy;
		...
		else
		{
			...
			//if(!Size && (
			//	!m_DummyInput.m_ViewDir &&
			//	!m_DummyInput.m_Jump &&
			//	!m_DummyInput.m_Hook &&
			//	!m_DummyInput.m_AimX &&
			//	!m_DummyInput.m_AimY &&
			//	!m_DummyInput.m_NextWeapon &&
			//	!m_DummyInput.m_PrevWeapon &&
			//	!m_DummyInput.m_PlayerFlags &&
			//	!m_DummyInput.m_WantedWeapon &&
			//	!m_DummyInput.m_FCount
			//	
			//	))
			//	return;
...
void CClient::Disconnect()
{
	...
	if(m_DummyConnected)
		DummyDisconnect(0);
		/*DummyDisconnect(g_Config.m_ClNamePlatesBroadcastATH ? "> Sash < " : 0);*/
	if(m_State != IClient::STATE_OFFLINE)
		DisconnectWithReason(0);
		/*DisconnectWithReason(g_Config.m_ClNamePlatesBroadcastATH ? "> Sash < " : 0);*/
}
		...
		else if(Msg == NETMSG_SNAP || Msg == NETMSG_SNAPSINGLE || Msg == NETMSG_SNAPEMPTY)
		{
			...
			if(GameTick >= m_CurrentRecvTick[g_Config.m_ClDummy])
			{
				...
				if(m_SnapshotParts == (unsigned)((1<<NumParts)-1))
				{
					...
					// ack snapshot
					if(g_Config.m_ClFakePingInc != 0)
					{
						//GameTick = (GameTick + g_Config.m_ClAveragePing*2/40) - g_Config.m_ClFakePing*2/40;
						GameTick = GameTick - g_Config.m_ClFakePingInc * 2 / 40;
						GameTick += GameTick % 2; // Be sure it ends with 0,2,4,6 or 8
					}
					if (g_Config.m_ClFakePingDec != 0)
					{
						//GameTick = (GameTick + g_Config.m_ClAveragePing*2/40) - g_Config.m_ClFakePing*2/40;
						GameTick = GameTick + g_Config.m_ClFakePingDec * 2 / 40;
						GameTick += GameTick % 2; // Be sure it ends with 0,2,4,6 or 8
					}
			...
			char aBuf[256];
			str_format(aBuf, sizeof(aBuf), "WTF!!!!! GOT Unbknown msg %s", "");
			m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, "sashSniff", aBuf);
		}
...
//#endif
	...
	*/
	...
	try {
		...
		char aBuf[512];
		str_formatb(aBuf, "How embarrassing, Sash encountered a fatal error and crashed :(\n"
...
//std::string CClient::GetCurrentMapStd()
//{
//	CALLSTACK_ADD();
//
//	return std::string( m_aCurrentMap);
//}
//
