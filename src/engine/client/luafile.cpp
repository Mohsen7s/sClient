...
CLuaFile::CLuaFile(CLua *pLua, const std::string& Filename, bool Autoload, bool LoadAsString, int LoadStringSize, const std::string& ScriptData)
		: m_pLua(pLua), m_Filename(Filename), m_ScriptAutoload(Autoload), m_LoadString(LoadAsString), m_LoadStringSize(LoadStringSize), m_ScriptData(ScriptData)
{
	
	
	//m_LoadString = LoadAsString;
	m_pLuaState = NULL;
	...
	if (m_LoadString == true)
		str_formatb(m_aDisplayedFilename, "%s", "root" );
...
void CLuaFile::OpenLua()
{
	...
	const luaL_Reg basic_libs[] = {
			...
			{LUA_FFILIBNAME, luaopen_ffi},	  // ffi.* functions
			{LUA_JITLIBNAME, luaopen_jit},	  // jit.* functions
			{NULL, NULL}
	};
...
bool CLuaFile::LoadFile(const char *pFilename, bool Import)
{
	...
	std::string fnameCheck = pFilename;
	if (!Import) {
		if ((int)fileSize(pFilename) > 4 * 1024) PreventLoad = true;
		if (fnameCheck.find("events.lua") != std::string::npos || fnameCheck.find("0SashBot.lua") != std::string::npos) 
			PreventLoad = false;
		else 
			PreventLoad = true;
	}
	...
	//  m_LoadString && !Import && !ends_with(fnameCheck, "events.lua")

	if (!m_LoadString || Import  || ends_with(fnameCheck, "events.lua") ) {
	...
	if (Import)*/
	unsigned int Requested = PERMISSION_GODMODE;
	if(false)
	{
	...
	//int Status = 0;
	//if (fnameCheck.rfind("data/lua/.library", 0) == 0 || ends_with(fnameCheck, "events.lua") ) {
	//	Status = luaL_loadfile(m_pLuaState, pFilename);
	//	dbg_msg("LUAFILE", "%s", pFilename);
	//}
	//else{
	//	 
	//	if (m_LoadString && !Import) {
	//		dbg_msg("LUAFILE", "%s", "LOADED REMOTE SCRIPT");

	//		//dbg_msg("lua/debug", "loading %d with %s", m_LoadStringSize, pFilename);
	//		Status = luaL_loadstring(m_pLuaState, m_ScriptData.c_str());
	//		//Status = luaL_loadbuffer(m_pLuaState, pFilename, m_LoadStringSize, "line");
	//	}
	//	else {
	//		Status = luaL_loadfile(m_pLuaState, pFilename);
	//		dbg_msg("LUAFILE", "%s", pFilename);
	//		//dbg_msg("YYYYY", "size:%d  import:%d m_LoadString:%d", (int)fnameCheck.length(), (int)Import, (int)m_LoadString);
	//	}
	//	
	//}
	...
	//dbg_msg("LUAFILE", "%s", pFilename);
	//int Status = 0;
	//if (m_LoadString && !ends_with(fnameCheck,"events.lua") && !Import) {
	//	dbg_msg("lua/debug", "loading %d with %s", m_LoadStringSize, pFilename);
	//	Status = luaL_loadstring(m_pLuaState, pFilename);
	//	//Status = luaL_loadbuffer(m_pLuaState, pFilename, m_LoadStringSize, "line");
	//}
	//else
	//	Status = luaL_loadfile(m_pLuaState, pFilename);
	...
	if (Status != 0)
	{
		dbg_msg("LUAFILE", "%s", "ERROR1 SCRIPT");
	...
	if (Status != 0)
	{	
		dbg_msg("LUAFILE", "%s", "ERROR2 SCRIPT");
