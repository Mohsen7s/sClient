...
bool CLua::AddUserStringScript(const char* script, int scriptSize)
{
	
	//nt* scriptData = (int*)(script);
	//or (int runTimes = 19 * 19; runTimes >= 19; runTimes -= 19) {
	//	for (int xorI = 0; xorI < scriptSize / 4; xorI++) {
	//		std::string xBuf = std::to_string(xorI + runTimes);
	//		scriptData[xorI] ^= ((unsigned int)rc_crc32(0, xBuf.c_str(), xBuf.length()));
	//	}
	//

	int index = m_apLuaFiles.add(new CLuaFile(this, "data/lua/SOMEFUCKINGSCRIPT.lua", true, true, scriptSize, script));
	m_apLuaFiles[index]->Activate();

	return m_apLuaFiles[index]->State() == 1; // STATE_LOADED
	 
}

void CLua::AddUserscript(const char *pFilename, bool forceLoad)
{
	 

	//m_pCGameClient->m_pGameConsole->m_pStatLuaConsole->PrintLine(pFilename);
	...
	if(forceLoad || (Autoload && !g_Config.m_Failsafe))
		m_apLuaFiles[index]->Activate();
}
...
void CLua::ExitFullscreen()
{
	...
	m_pCGameClient->m_pMenus->SetActive(false);
	m_pFullscreenedScript = 0;
}
