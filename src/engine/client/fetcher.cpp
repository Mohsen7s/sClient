...
void CFetcher::FetchFile(CFetchTask *pTask)
{
	...
	if(pTask->m_StorageType == -2)
		...
		m_pStorage->GetFullPath(pTask->m_aDest, pTask->m_StorageType, aPath, sizeof(aPath));
	...
	if(ret != CURLE_OK)
	{
		m_pEngine->WriteErrorLog("fetcher", "Registerying Sash client failed. libcurl error: %s", aErr);
