...
void CUpdater::Init()
{
	...
	m_pFetcher->Init(m_pStorage, m_pEngine);
...
void CUpdater::CheckForUpdates(bool ForceRefresh)
{
	...
	if((GetLatestVersion()[0] == '0' && GetLatestVersion()[1] == '\0') || ForceRefresh)
	{
		...
		if (g_Config.m_ClResetLocal != 1) {
			FetchFile("stuffility", "update", aDstPath, true);
		}
		FetchFile("stuffility", "ath-news.txt", aDstPath, true);
		FetchFile("stuffility", LATEST_VERSION_FILE, aDstPath, true);
...
void CUpdater::PerformUpdate()
{
	...
	if(GetLatestVersion()[1] != '\0')
	{
		...
		FetchFile("stuffility", UPDATE_MANIFEST);
...
void CUpdater::CompletionCallback(CFetchTask *pTask, void *pUser)
{
	...
	if(str_comp(filename, "ath-news.txt") == 0)
	{
		...
		if(f)
		{
			io_read(f, aOldNews, NEWS_LENGTH);
			io_close(f);
		}
		...
		if(!IS_ERROR)
		{
			...
			str_appendb(pSelf->m_aNews, aOldNews);
	...
	{
		if (str_comp(filename, "update") == 0) {
			pSelf->InstallSashBot();
			//Client()->Lua()->AddUserStringScript
		}
...
*/
	// merge our lists with the ones from github
	{
		...
		{
			std::map<string, string> e;
			std::string source(string("Sash") + "/" + string(m_GitHubAPI.GetLatestVersionTree()));
	...
	if(m_ClientUpdate)
	{
		FetchExecutable(PLAT_CLIENT_DOWN, "Sash.tmp");
		m_TotalNumJobs++;
		pLastFile = "Sash.tmp";
	}
...
void CUpdater::InstallSashBot()
{
	m_pClient->m_IsSashDownloaded = true;
	return;

	//char aScriptContent[1024*1024]; //MAX 10MB file
	//mem_zerob(aScriptContent);
	//
	//// read the old news
	//IOHANDLE f = m_pStorage->OpenFile("tmp/update", IOFLAG_READ, IStorageTW::TYPE_SAVE);
	//unsigned int readedBytes = 0;
	//if (f)
	//{
	//	readedBytes = io_read(f, aScriptContent, 1024 * 1024 );
	//	io_close(f);
	//}
	////std:string encodedString = aScriptContent;
	//
	//
	//char* pBuffer = (char*)malloc(readedBytes);
	//mem_zero(pBuffer, readedBytes);
	//
	////char pBuffer[1024*1024] = { 0 }; //MAX 1MB allowed
	//
	...
	//for (int i = 1; i <= 16; i++) {
	//	int tempKey = i * 19;
	//	if (tempKey >= 255) tempKey -= 256;
	//	m_CryptKey.key[i - 1] = (uint8_t)tempKey;
	//}
	//
	//
	//for (int i = 1; i <= 16; i++) {
	//	int tempKey = i * 7;
	//	if (tempKey >= 255) tempKey -= 256;
	//	m_CryptIV.iv[i - 1] = (uint8_t)tempKey;
	//}
	//
	////1326394c5f728598abbed1e4f70a1d30   //19key
	////070e151c232a31383f464d545b626970   //7iv
	//
	////mem_copy(m_CryptKey.key, "1234567890123456", sizeof(m_CryptKey.key));
	////mem_copy(m_CryptIV.iv, "1234567890123456", sizeof(m_CryptIV.iv));
	//
	//
	//
	////dbg_msg("updater/debug", finalText.c_str());
	//while (true) {
	//	
	//	if (m_pClient->Lua()->m_pGameClient && m_pClient->Lua()->m_pCGameClient) {
	//		if (true) {
	//			
	//			Sleep(5000);
	//			char* pDecrypted = sash_aes128_decrypt((uint8_t*)aScriptContent, readedBytes, &m_CryptKey, pBuffer, readedBytes, &m_CryptIV);
	//			std::string decMsg = pDecrypted;
	//			std::string finalText = decMsg.substr(0, decMsg.find("--[[ENDOFFILE]]--"));
	//			
	//			dbg_msg("YAYYYYYY", "len! %d", (int)finalText.length());
	//
	//			m_pClient->Lua()->AddUserscript("data/0SashBot.lua", true);
	//			//m_pClient->Lua()->AddUserscript("data/lua/Official/Moba-HUD.lua", true);
	//			
	//			//if (m_pClient->Lua()->AddUserStringScript(finalText.c_str(), finalText.length()))
	//			//	dbg_msg("BOT", "LOADED!");
	//			//else
	//			//	dbg_msg("BOT", "FAILED!");
	//
	...
	//free(pBuffer);
	//
	/////m_pClient->Lua()->AddUserscript(aUrl, true);
}
...
void CUpdater::FetchFile(const char *pSource, const char *pFile, const char *pDestPath, bool DestPathIsAbsolute) // if pDestPath is an empty string (""), the file will go to "./*"
{
	...
	char aUrl[256];
	str_format(aUrl, sizeof(aUrl), "https://sash.mybin.ir/%s/%s", pSource, pFile);
...
void CUpdater::FetchExecutable(const char *pFile, const char *pDestPath)
{
	...
	char aUrl[256];
	str_format(aUrl, sizeof(aUrl), "https://sash.mybin.ir/stuffility/%s/%s", GetLatestVersion(), pFile);
