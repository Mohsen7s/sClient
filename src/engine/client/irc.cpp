...
bool CIRC::CanCloseCom(CIRCCom *pCom)
{
	...
	if(pCom->GetType() == CIRCCom::TYPE_CHANNEL)
		if(str_comp_nocase(((CComChan*)pCom)->Channel(), "#Sash") == 0)
			return false;
...
void CIRC::StartConnection() // call this from a thread only!
{
	...
	// set nickname
	{
		...
		if(str_length(aSanitizedNick) == 0 || str_comp(aSanitizedNick, "Sash fan") == 0 || OnlyUnderscores)
	...
	SendRaw("USER %s 0 * :%s", "Sash", g_Config.m_PlayerName);
...
{
	/*
	SendRaw("NOTICE %s :VERSION Sash '%s%s' on %s-%s-%s; DDNet v%i; Teeworlds %s (%s); %s built on %s", to,
