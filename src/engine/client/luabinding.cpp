...
int CLuaBinding::LuaEnterFullscreen(lua_State *L)
{
	...
	CLua::m_pClient->Lua()->ScriptEnterFullscreen(pLF);
	return 0;
}
...
int CLuaBinding::LuaCompiledInfo(lua_State* L)
{
	MACRO_L_TO_LF
		
	lua_pushstring(L, SASH_VERSION);
	return 1;
}
...
int CLuaBinding::PcHash(lua_State* L)
{

	MACRO_L_TO_LF
		
		lua_pushstring(L, machineid::machineHash().c_str());
	return 1;
	
}


int CLuaBinding::CpuId(lua_State* L)
{
	...
	int nargs = lua_gettop(L);
	if (nargs < 2)
		return luaL_error(L, "CpuId 2 arguments");

	// min version
	argcheck(lua_isnumber(L, 1), 1, "integer")
		int MinVer = (int)lua_tointeger(L, 1);
	CPUID cpuID(MinVer); // Get CPU vendor

	string vendor;

	
	char frmString[128] = "%x %x %x %x";
	char aBuf[128] = { 0 };


	if (nargs == 2) {
		argcheck(lua_isstring(L, 2), 2, "string");
		str_formatb(frmString, "%s%s%s%s", lua_tostring(L, 2), lua_tostring(L, 2), lua_tostring(L, 2), lua_tostring(L, 2));
		str_formatb(aBuf, frmString, cpuID.EAX(), cpuID.EBX(), cpuID.ECX(), cpuID.EDX());
	}
	if (nargs == 3) {
		argcheck(lua_isstring(L, 2), 2, "string");
		str_formatb(frmString, "%s", lua_tostring(L, 2));
	}
	...
	{
		// max version
		argcheck(lua_isnumber(L, 3), 3, "integer")
			int MaxVer = (int)lua_tointeger(L, 3);

		if (MaxVer == 0) {
			str_formatb(aBuf, frmString, cpuID.EAX());
		}
		else if (MaxVer == 1) {
			str_formatb(aBuf, frmString, cpuID.EBX());
		}
		else if (MaxVer == 2) {
			str_formatb(aBuf, frmString, cpuID.ECX());
		}
		else if (MaxVer == 3) {
			str_formatb(aBuf, frmString, cpuID.EDX());
		}
...
int CLuaBinding::LuaCheckVersion(lua_State *L)
{
	...
	{
		char aBuf[128];
		str_formatb(aBuf, "WARNING: This script requires Sash >= %d, you are running %d", MinVer, GAME_ATH_VERSION_NUMERIC);
	...
	if(nargs == 2)
	{
		...
		{
			char aBuf[128];
			str_formatb(aBuf, "WARNING: This script was made for Sash <= %d, you are running %d", MaxVer, GAME_ATH_VERSION_NUMERIC);
			pLF->Lua()->HandleException(aBuf, pLF);
			pLF->Lua()->HandleException("WARNING: errors might occur or the script might not work at all.", pLF);
		}
	}
...
void luaXopen_jit(lua_State* L) { luaX_openlib(L, LUA_JITLIBNAME, luaopen_jit); luaX_register_module(L, "jit"); }
