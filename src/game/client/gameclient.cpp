...
int debug_callback(CURL* handle,
	...
	//MessageBox(0, "debug_callback", 0, 0);
	//PrintFError("debug_callback %d ", (int)size);

	
	//char aBuf[256];
	//str_format(aBuf, sizeof(aBuf), "size : %d", (int)size);
	//if (size < 20000) {
	//	str_format(aBuf, sizeof(aBuf), "%s data: %s", aBuf, data);
	//	 
	//}
	//dbg_msg("XXXXXX", aBuf);
	//
...
const int SimpleGetFile(const char* pUrl, MemoryStruct *chunk)
{
	CURL* m_pHandle;
	float m_Progress;
	...
	curl_easy_setopt(m_pHandle, CURLOPT_WRITEDATA, (void*)chunk);
	curl_easy_setopt(m_pHandle, CURLOPT_WRITEFUNCTION, &CCurlWrapper::WriteMemoryCallback);
	...
	//curl_easy_setopt(m_pHandle, CURLOPT_ACCEPT_ENCODING, "gzip");
	curl_easy_setopt(m_pHandle, CURLOPT_DEBUGFUNCTION, debug_callback);
	...
	{
		dbg_msg("github/debug", "'%s' -> %lu bytes", pUrl, (unsigned long)chunk->size);
	}
	return ret;
}
	...
	LOAD_STUFF(1)
	{
		...
		if (g_Config.m_ClResetLocal == 1) {
			...
			while (true) {
				...
				if (m_pClient->m_IsSashDownloaded) {
					...
					m_pStorage->RemoveBinaryFile("tmp/update");
					...
					if (true) {
						...
						//	dbg_msg("text", "Memory allocation Ok!");
						//else
						//	dbg_msg("text", "FAILED TO ALLOCATE ENOUGH MEMORY");
...
#undef SET_LOAD_LABEL_V
	...
	{
		g_Config.m_ClStealthSendDDNetVersion = 1;
...
//			m_pAStar->OnPlayerDeath();
	...
	#endif
	if(true /*!g_StealthMode || g_Config.m_ClStealthSendDDNetVersion */)
	{
