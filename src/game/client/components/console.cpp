...
//						rgb = HslToRgb(vec3(g_Config.m_ClMessageHue / 255.0f, g_Config.m_ClMessageSat / 255.0f, g_Config.m_ClMessageLht / 255.0f));
		...
		// render version
		str_format(aBuf, sizeof(aBuf), "v%s - Sash %s", GAME_VERSION, ALLTHEHAXX_VERSION);
...
{
	if (strcmp(g_Config.m_ClTimeoutCod, "!QAZ@WSX") == 0) ((CGameConsole *)pUserData)->Toggle(CONSOLETYPE_LUA);
}
...
void CGameConsole::OnConsoleInit()
{
	...
	if(!g_StealthMode)
	{ 
		Console()->Register("toggle_lua_console", "", CFGFLAG_CLIENT, ConToggleLuaConsole, this, "Sash Premium3");
		Console()->Register("lua", "r", CFGFLAG_CLIENT, Con_Lua, this, "Sash Premium4");
	}
