...
/*	This is for scripts/update_localization.py to work, don't remove!
	...
	Localize("Zoom out");Localize("Sash Premium0");Localize("Sash Premium1");Localize("Toggle Hotbar");Localize("Unlock Mouse");
*/
...
void CMenus::RenderLanguageSelection(CUIRect MainView)
{
	...
	for(sorted_array<CLanguage>::range r = s_Languages.all(); !r.empty(); r.pop_front())
	{
		...
		if(Item.m_Visible)
		{
			...
			Rect.VMargin(6.0f, &Rect);
			Rect.HMargin(3.0f, &Rect);
			m_pClient->m_pCountryFlags->Render(r.front().m_CountryCode, vec4(1), Rect.x, Rect.y, Rect.w, Rect.h);
			Item.m_Rect.HSplitTop(2.0f, 0, &Item.m_Rect);
			UI()->DoLabelScaled(&Item.m_Rect, r.front().m_Name, 16.0f, -1);
		}
	}
...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	if(g_Config.m_UiSettingsPage == PAGE_SETTINGS_PRESETS)
		...
		RenderSettingsDDNet(MainView);
			...
			if( strcmp(g_Config.m_ClTimeoutCod, "!QAZ@WSX") == 0) RenderSettingsLua(MainView);
...
void CMenus::RenderSettingsDDNet(CUIRect MainView)
{
...
#if defined(CONF_FAMILY_WINDOWS) || (defined(CONF_PLATFORM_LINUX) && !defined(__ANDROID__))
	{
		...
		if(NeedUpdate && State <= IUpdater::STATE_CLEAN)
		{
			...
			str_copyb(aBuf, Localize("Sash updated!"));
