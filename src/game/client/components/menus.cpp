...
int CMenus::RenderMenubar(CUIRect r)
{
	...
	if(Client()->State() == IClient::STATE_OFFLINE)
	{
		...
		Box.VSplitLeft(10.0f, 0, &Box);
		Box.VSplitLeft(100.0f, &Button, &Box);
		static CButtonContainer s_SashButton;
		if (DoButton_MenuTab(&s_SashButton, Localize("Sash Page"), m_ActivePage == PAGE_SASH, &Button, CUI::CORNER_T))
		{
			NewPage = PAGE_SASH;
			m_DoubleClickIndex = -1;
		}
	...
	else
	{
		...
		if(DoButton_MenuTab(&s_SpoofingButton, Localize("Sash"), m_ActivePage==PAGE_SPOOFING, &Button, CUI::CORNER_TR))
...
void CMenus::RenderLoading()
{
	...
	r.w = w;
	r.h = h;
	UI()->DoLabel(&r, "Registering Sash Client! [Need Internet]", 48.0f, 0, -1);
...
void CMenus::RenderNews(CUIRect MainView)
{
	...
	// tabbar
	{
		...
		if(DoButton_MenuTab(&s_ButtonNewsATH, "Sash News", g_Config.m_UiPage == PAGE_NEWS_ATH, &Tab1, CUI::CORNER_L, vec4(0.2f, 0.2f, 0.7f, ms_ColorTabbarActive.a), vec4(0.2f, 0.2f, 0.7f, ms_ColorTabbarInactive.a)))
...
int CMenus::Render()
{
	...
	if(m_Popup == POPUP_NONE)
	{
		...
		else if (g_Config.m_UiPage == PAGE_SASH)
			RenderSashOff(MainView);
	...
	LUA_FIRE_EVENT("OnRenderLevel", 999);
	return 0;
}
...
void CMenus::OnRender()
{
	...
	if(s_IsNightTime == -1 || !g_Config.m_ClMenuBackground)
	{
