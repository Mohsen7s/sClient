...
static void logger_stdout(const char *line)
{
	...
	fflush(stdout);
#if defined(__ANDROID__)
	__android_log_print(ANDROID_LOG_INFO, "Sash", "%s", line);
#endif
}
...
char* sash_aes128_decrypt(uint8_t* data, unsigned data_size, const AES128_KEY* key, char* buffer, unsigned buffer_size, AES128_IV* out_iv)
{
	int i;
	...
	AES128_CBC_decrypt_buffer(output_buffer, data , data_size , key->key, out_iv->iv);
